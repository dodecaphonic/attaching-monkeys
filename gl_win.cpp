...
void OpenGLWindow::da_on_realize()
{
  ...
  Glib::RefPtr<Gdk::Screen> canvas = this->get_screen();

  m_camera.SetMonitorPixelSize(canvas->get_width(), canvas->get_height());
  m_camera.SetMonitorWorldSize(canvas->get_width_mm(), canvas->get_height_mm());
  int dpi = static_cast<int>(gdk_screen_get_resolution(canvas->gobj()));
  if (dpi < 0)
    dpi = 96;
  m_camera.SetMonitorPixelInc(dpi, dpi);
  m_camera.Resize(Canvasw, Canvash);
  
  */

  if (!gdk_gl_drawable_gl_begin(glDrawable, glContext))
  {
    //fprintf(stderr, "Não foi possível iniciar o OpenGL.");
    gtk_main_quit();
    exit(0);
  }
...
bool
OpenGLWindow::vai_tomar_no_cu(GdkEventButton *event)
{
        printf("Tomei\n");
        return true;
}
